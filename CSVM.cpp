...
void CSVM::train(const vector<t_sample>& v_samples, const vector<t_label>& v_labels)
{
	...
	if( size_sample != size_label )
		throw EError("[CSVM::Train] Index and sample size must be equal.");

	int len = int(v_samples[0].size());

	const char *error_msg = svm_check_parameter(&prob,&m_param);
	if(error_msg)
		throw EError("[CSVM::Train] Error occurs as " + string(error_msg));
...
{
	if( nfold < 3 )
		throw EError("[CSVM::CrossValidation] Fold value must be larger than 2");

	if(m_param.svm_type == EPSILON_SVR || m_param.svm_type == NU_SVR)
		throw EError("[] System not adapted for these methods.");
...
{
	if(not m_b_trained)
		throw EError("[CSVM::Write] First train the SVM.");
...
void CSVM::read(const UPath& pthinput)
{
	if( (m_model=svm_load_model(pthinput)) == 0 )
		throw EError("[CSVM::Read] SVM file can't open., " + pthinput);

	m_b_trained = true;
}

void CSVM::write(const UPath& pthoutput)
{
	if(not m_b_trained)
		throw EError("[CSVM::Write] First train the SVM.");

	if(svm_save_model(pthoutput, m_model))
		throw EError("[CSVM::Write] SVM file can't open., " + pthoutput);
}
